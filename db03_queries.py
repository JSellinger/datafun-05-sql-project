...
def run_queries():
    ...
    aggregation_queries = read_sql_file(queries_folder.joinpath("02_aggregation.sql"))
    for query in aggregation_queries:
        aggregate_df = execute_sql_query(db_file_path, query)
        print(f'Aggregation Results: \n{aggregate_df}')

    # Run filtering query
    filtering_queries = read_sql_file(queries_folder.joinpath("03_filter.sql"))
    for query in filtering_queries:
        filter_df = execute_sql_query(db_file_path, query)
        print(f'Filtered Results: \n{filter_df}')

    # Run sorting query
    sorting_queries = read_sql_file(queries_folder.joinpath("06_sorting.sql"))
    for query in sorting_queries:
        sort_df = execute_sql_query(db_file_path, query)
        print(f'Sorted Results: \n{sort_df}')

    # Run grouping query
    grouping_queries = read_sql_file(queries_folder.joinpath("04_group_by.sql"))
    ...
    # Run join query
    joining_queries = read_sql_file(queries_folder.joinpath("05_join.sql"))
